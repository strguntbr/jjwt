...
public class JwtMap implements Map<String, Object> {
    ...
    static {
        ...
        if (Classes.isAvailable("io.jsonwebtoken.impl.datehandler.Jsr310DateSetterAndGetter")) {
            Class<?> jsr310DateSetterAndGetterClass = Classes.forName("io.jsonwebtoken.impl.datehandler.Jsr310DateSetterAndGetter");
            Object jsrDateSetterAndGetter;
            try {
                jsrDateSetterAndGetter = jsr310DateSetterAndGetterClass.newInstance();
            } catch (InstantiationException | IllegalArgumentException | IllegalAccessException e) {
                throw new IllegalStateException("Could not load Jsr310DateSetterAndGetter. Please make sure that java.time.* classes are on the classpath when you try to enable JSR310 date class handling");
            }
            DATE_SETTERS.add((DateSetter) jsrDateSetterAndGetter);
            DATE_GETTERS.add((DateGetter) jsrDateSetterAndGetter);
        }
    }
